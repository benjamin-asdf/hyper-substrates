...
(defn- setup-2d-nav
  ...
  (let [initial-state (-> user-settings
                          (select-keys [:position :zoom :mouse-buttons :modifiers])
...
(defn -modifier-fit? [modifier-set current-modifiers]
  (or (not modifier-set) (= modifier-set current-modifiers)))
...
(defn navigation-2d
  ...
  (let [user-settings (:navigation-2d options)
        ...
        user-mouse-dragged
          (:mouse-dragged options (fn [state _] state))
        user-mouse-wheel (:mouse-wheel options
                                       (fn [state _] state))
        modifiers (:modifiers user-settings {})
    ...
    (assoc options
      ...
      :mouse-wheel
        (fn [state event]
          (if (and (q/key-pressed?)
                   (-modifier-fit? (:mouse-wheel modifiers)
                                   (q/key-modifiers)))
