...
(defn lively-eye
  []
  (let [eye-state (atom {:target-pos (e/* [(q/width)
                                           (q/height)]
                                          (q/random-2d))})]
    [(let [pos [(+ (/ (q/width) 2) 100) (/ (q/height) 2)]
           target-pos (e/* [(q/width) (q/height)]
                           (q/random-2d))
           ;; gamma frequency
           ;; say 40 Hz
           frequency 1]
       (-> (lib/->entity :circle
                         {:color :black
                          ...
                          :transform
                          (lib/->transform pos 50 50 1)})
           (lib/live [:vibrate (vibrate pos frequency)])

           (lib/live [:change-target
                      (lib/every-n-seconds
                       0.5
                       (fn [e s k]
                         (assoc e
     ...
     ;; (let [pos [(- (/ (q/width) 2) 100) (/
     ;; (q/height) 2)]
     ;;       target-pos (e/+ (e/* (q/random-2d) 10)
     ;;       pos)
     ;;       ;; gamma frequency
     ;;       ;; say 40 Hz
     ;;       frequency 40]
     ...
     ;;         :circle
     ;;         {:color :black
     ;;          :target-pos target-pos :transform
     ;;            (lib/->transform target-pos 50 50
     ;;            1)})
     ;;       (lib/live [:vibrate (vibrate pos
     ;;       frequency)])
     ;;       (lib/live [:change-target
     ;;                  (lib/every-n-seconds
     ;;                    (/ 1 5)
