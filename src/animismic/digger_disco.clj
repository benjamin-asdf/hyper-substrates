...
(defn ->gridoids
  ...
  (let [spacing 50]
    (-> (lib/->entity
          :gridoids
          {:unique-id :grid-balls
           :alpha 0.5
           :color (lib/->hsb defs/white)
           :stroke-weight 0
           :draw-element (fn [e elm] (q/rect 0 0 20 20))
           :draw-functions
             {:f (fn [e]
                   ;; (q/stroke-weight 1)
                   (q/with-stroke
                     ...
                     (q/with-fill
                       (lib/with-alpha
                         ...
                         (:alpha e))
                       (draw-grid-2 e (:elements e)))))}
           ;; :on-elements
           ;; (repeatedly (rand-int () ))
           :elements
             (let [w (q/width)
                   h (q/height)
                   x (long (Math/floor (/ w spacing)))
                   y (long (Math/floor (/ w spacing)))]
               (torch/zeros [x y]))
           :get-elm (fn [e] (py.. e item))
           :kinetic-energy 0.1
           :particle? true
           :spacing 50
           :transform (lib/->transform [50 50] 1 1 1)})
        (lib/live (fn [e s k]
                    (update-in
                      e
                      [:alpha]
                      (fn [a]
                        (lib/sine-wave 0.2 (q/millis))))))
        (lib/live (fn [e s k]
                    (update-in e
                               [:stroke-weight]
                               (defn gupdate-stroke
...
(defn ->gridoids-2
  ...
  (let [spacing 50]
    (keep
     ...
     (mapcat identity
             (doall
              (for [x (range 0 (q/width) spacing)]
                (doall
                 (for [y (range 0 (q/height) spacing)]
                   (when (zero? (fm.rand/flip 0.995))
                     (->
                      (lib/->entity
                       (rand-nth [:circle :rect])
                       {:color defs/white
                        :gridoid? true
                        :initial-pos [x y]
                        :intensity-factor 1
...
(comment
                               ...
                               (lib/with-alpha
                                 (lib/->hsb
                                  ((rand-nth
                                   ...
                                   ;; :black])
                                   defs/color-map))
                                 (* 0.23 (lib/sine-wave 0.2 (q/millis))))))))))])))
           ...
           (fn [e]
             (if (= (:kind e) :circle)
               (assoc
                e
                :stroke
                ((rand-nth [:cyan nil nil nil :black])
                 defs/color-map)))))))])))
     ...
     (lib/live
      ...
      [:flip-circles
       (lib/every-now-and-then
        ...
        (fn [s k]
          s
          ;; (let [stroke (rand-nth [:cyan nil nil nil
          ;;                         :black])]
          ;;   (lib/update-ents
          ;;    s
          ;;    (fn [e]
          ;;      (if (= (:kind e) :circle)
          ;;        (assoc e :stroke (stroke defs/color-map))
          ;;        e))))
          ))])))
  ...
  (defn new-gridoids
    [state]
    (-> (lib/update-ents state
                         (fn [e]
                           (if (:intensity-factor e)
                             (assoc e :kill? true)
                             e)))
        (lib/append-ents (->gridoids-2))))

  (evtq/append-event! new-gridoids)
    ...
    (lib/append-ents
     ...
     (map
      (fn [e]
        ...
        ;;      [:transform :scale] 1)
        ;;     (assoc :color :cyan)
        ;;     (assoc :kind (get {:circle :rect
        ;;                        :rect :circle}
        ;;                       (:kind e))))
        )
      (filter :gridoid? (lib/entities state)))))
  ...
  (defn update-entities! [index f]
    (evtq/append-event!
     (fn [state]
       (let [ents (map f (lib/entities-by-index state index))]
         (lib/update--entities
          state
          #(merge % (into {} (map (juxt :id identity)) ents)))))))

  (update-entities! :g-confetti #(assoc % :kill? true)))
