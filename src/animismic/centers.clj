...
(defmethod lib/setup-version :braitenberg-centers
  [state]
  (->
    state
    (lib/append-ents
     [
      ...
      (->
       (blerp-retina {:color ;; (:heliotrope defs/color-map)
                      (:ice-cyan defs/color-map)
                      :grid-width grid-width
                      :particle-field-opts
                      {:attenuation-factor 1.5
                       :decay-factor 0.05
                       :vacuum-babble-factor (/ 5 1000)}
                      :particle-id :ice
                      :pos [50 50]
                      :spacing 20})
