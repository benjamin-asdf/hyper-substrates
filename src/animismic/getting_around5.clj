                 ...
                 (lib/append-ents
                  ...
                  [(let [e (lib/->circular-shine-1
                     ...
                     (->
                      e
                      (assoc :color
                             ((rand-nth [:black :hit-pink :cyan :heliotrope ]) defs/color-map)
                             )
                      ;; (assoc :stroke-weight 3)
                      ;; (assoc :stroke (:color ray))
                      (assoc
                         ...
                         (* 2
                            (+ 1
                               (:intensity-factor
                                ...
                                0))))])
                      (assoc :lifetime
                             (lib/normal-distr 10 (Math/sqrt 5)))))])}))])
...
(defmethod lib/setup-version :getting-around-5 [state] (setup-internal state))
            ...
            {:body {:color (:navajo-white defs/color-map)
             ...
             :components
               [[:cart/entity :_
                ...
                [:brain/connection :_
                 {:destination [:ref :ma]
                  :f (lib/->weighted (if angry? 1 -5))
                  :hidden? true
                  :source
                    (if angry? [:ref :sb] [:ref :sa])}]
                [:brain/connection :_
                 {:destination [:ref :mb]
                  :f (lib/->weighted (if angry? 1 -5))
                  :hidden? true
                  :source
                  (if angry? [:ref :sa] [:ref :sb])}]]})]
    cart))
                    ...
                    {:body
                     ...
                     [[:cart/motor :ma
                      ...
                      [:brain/connection :_
                       {:decussates? true
                        :destination [:ref :ma]
                        :on-update-map {:gain (fn [e s k] (assoc-in e [:transduction-model :gain] #(* 10 %)))}
                        :f lib/excite
                        :hidden? true
                        :source [:ref :sb]}]
...
(do (sketch {:background-color 0
    ...
    (swap! lib/event-queue (fnil conj []) add-ray-source)
    (swap! lib/event-queue (fnil conj [])
      (fn [s] (lib/append-ents s (vehicle))))
  ...
  (swap! lib/event-queue (fnil conj [])
         (fn [s]
           (lib/update-ents
            s
            (fn [e]
              (if-let [model (:transduction-model e)]
                (if-not (:sensor? ((lib/entities-by-id s)
                                   (:source model)))
                  e
                  (-> e
                      ;; (assoc :color (:red defs/color-map))
                      (assoc-in
     ...
     (lib/update-ents
      ...
      (fn [e]
        (if-not
            (:body? e)
            e
            (update-in e [:transform :scale] #(* 1.1 %)))))
     )))
