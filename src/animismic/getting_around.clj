             ...
             (lib/live
              [:fly-towards-ray
               (lib/every-n-seconds
                ...
                (fn [e s k]
                  (if (zero? (rand-int 2))
                    (-> e (assoc :kinetic-energy 5))
                    (let
                        [ray-source (first (filter :ray-source? (lib/entities s)))]
                        (-> e
                            (lib/orient-towards (lib/position ray-source))
                            (update :acceleration (fnil + 0) 20)
                            (assoc :kinetic-energy 0))))))]))))))))
