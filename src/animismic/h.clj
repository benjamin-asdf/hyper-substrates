...
(defn concept-space
  ...
  (let [sdm (sdm/->sdm {:address-count (long 1e6)
    ...
    ;; 2. competition for blerps
    ;; 3. a kind of tensegrity:
    ;;    concepts want to be the sole concept
    ;;    but composit concepts recruit more blerps
...
(defn blerp-pump-update
  [e]
  (assoc-in e
    [:particle-field :decay-factor]
    (-> e
        :pump
        :decay-factor)))
...
;;
;; 1. low dim representation: a subgrid on the world
;; 2. high dim representation: ~population vectors
;;
;;
;;
;;
...
(comment
  ...
  ;; say 2 dimensions, up-down and left-right
  ;; now you can always split a grid into 4 corners, using population vectors
  ;; then you stack this, to zoom into a location
  ;; (or leave it wide)
  ;;

  ;; - you still also want to be able to make offsets I feel like
  ;; -
  ;;
  ...
  (map hdd/cleanup* (map (fn [i] (hd/permute-n pvec (- i))) (range 3)))

  (map
   hdd/cleanup-verbose
   (map (fn [i] (hd/permute-n pvec (- i))) (range 3)))



  ;; What do you do as blerp?
  ;; Your agenda is to activate the motor area in such a way that you are active again.
  ;;
  ;; This is the requirement for the motor area,
  ;; 1. be a resource to the effectors to move
  ;; 2. be a resource to the blerps to stabilize themselves
  ;;
...
(defmethod lib/setup-version :berp-retina-h
  [state]
  (-> state
      (lib/append-ents
       [
